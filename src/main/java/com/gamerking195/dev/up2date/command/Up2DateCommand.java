...
public class Up2DateCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if (!sender.hasPermission("u2d.command") && !sender.isOp()) {
            new MessageBuilder().addPlainText(MainConfig.getConf().getNoPermissionMessage()).sendToPlayersPrefixVariable((Player) sender);
            return true;
        }

        if (!MainConfig.getConf().isSetupComplete() && sender instanceof Player) {
        ...
        if (args.length == 0) {
            ...
            if (sender instanceof Player) {
                ...
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&', MainConfig.getConf().getPrefix() + "&dYou must be a player to setup the plugin!"));
                return true;
            }
            ...
            if (sender.hasPermission("u2d.manage") || sender.hasPermission("u2d.*") || sender.isOp()) {
                try {
                    MainConfig.getConf().reload();

                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', MainConfig.getConf().getPrefix() + "&dConfig reloaded!"));
                } catch (InvalidConfigurationException e) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', MainConfig.getConf().getPrefix() + "&dConfig reload failed, check console."));
